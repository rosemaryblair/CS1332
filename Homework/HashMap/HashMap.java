...
public class HashMap<K, V> {
    ...
    public void resizeBackingTable(int length) {
        if (length < size || length <= 0) {
            throw new IllegalArgumentException(
                "Desired length for resize must be positive & greater "
                    + "than/equal to the number of elements in the map.");
        }

        MapEntry<K, V>[] copy = table;
        table = (MapEntry<K, V>[]) new MapEntry[length];
        for (int i = 0; i < copy.length; i++) {
            MapEntry<K, V> temp = copy[i];
            if (temp != null && !temp.isRemoved()) { //not an empty space
                int k = Math.abs((temp.getKey().hashCode()) % table.length);
                if (table[k] == null) {
                    table[k] =
                        new MapEntry<K, V>(temp.getKey(), temp.getValue());
                } else {
                    while (table[k] != null) {
                        k = (k + 1) % table.length;
                    }
